...
DATASETS = [
    ...
    },
    {
            'name': 'boston_01',
            'prefix': 'boston_01_',
            'json_path': '/home/yanan/Downloads/Boston_01_PointCloud-Boston_01.json',
...
def point_to_pix(metadata, x, y, z, shift_degrees=180.0):
    list128 = metadata.beam_altitude_angles
    l = np.sqrt(x * x + y * y + z * z)
    if l == 0:
        return [0, 0]
    ...
    num128 = int(np.argmin(np.abs(np.array(list128) - sita128)))

    # 水平方向：用 arctan2 替代手动象限判断
    target_angle = -np.arctan2(y, x) * 180 / np.pi
    if target_angle < 0:
        target_angle += 360
    target_angle = (target_angle + shift_degrees) % 360
    num2048 = int((target_angle / 360.0) * IMG_WIDTH)

    return [num2048, num128]
...
# project_to_image 改为透传参数
def project_to_image(corners, metadata, shift_degrees=180.0):
    corners_pix = []
    for corner in corners:
        x, y, z = corner
        pix = point_to_pix(metadata, x, y, z, shift_degrees)
        corners_pix.append(pix)
    return np.array(corners_pix)
...
def process_dataset(dataset_config):
    ...
    shift_degrees = dataset_config.get('shift_degrees', 180.0)
    ...
    for frame_num, boxes in sorted(all_frames_boxes.items()):
        ...
        if boxes:
            for box in boxes:
                ...
                projected_corners = project_to_image(corners, metadata, shift_degrees)
