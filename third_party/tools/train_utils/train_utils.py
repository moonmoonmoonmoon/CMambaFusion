...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        model.train()
        if rank == 0 and accumulated_iter == 0:
            logger.info(f"[DTYPE] model param dtype: {next(model.parameters()).dtype}")
        ...
        if rank == 0 and accumulated_iter % 50 == 0:
            logger.info(f"[MEM] peak_allocated={peak_gb:.2f} GB, "
                        f"allocated={torch.cuda.memory_allocated() / 1024 ** 3:.2f} GB, "
                        f"reserved={torch.cuda.memory_reserved() / 1024 ** 3:.2f} GB")

        peak_mem = torch.cuda.max_memory_allocated() / 1024 ** 3
        if accumulated_iter < 5:
        ...
        if rank == 0:
            ...
            if use_logger_to_record:
                if accumulated_iter % logger_iter_interval == 0 or cur_it == start_it or cur_it + 1 == total_it_each_epoch:
                    ...
                    # 🔥 添加：从tb_dict提取alpha并记录
                    alpha_info = ''
                    if 'alpha' in tb_dict:
                        alpha_info = f" Alpha: {tb_dict['alpha']:.4f}"

                    logger.info(
                        'Train: {:>4d}/{} ({:>3.0f}%) [{:>4d}/{} ({:>3.0f}%)]  '
                        'Loss: {loss.val:#.4g} ({loss.avg:#.3g})  '
                        'LR: {lr:.3e}  '
                        f'{alpha_info} '  # 🔥 添加alpha
