...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    if hasattr(model, 'alpha'):
        # 检查alpha是否在优化器中
        alpha_in_optimizer = any(
            any(p is model.alpha for p in group['params'])
            for group in optimizer.param_groups
        )

        if not alpha_in_optimizer:
            # 添加alpha到优化器
            alpha_lr = optim_cfg.get('LR', 0.003) * optim_cfg.get('ALPHA_LR_MULTIPLIER', 10.0)
            optimizer.add_param_group({
                'params': [model.alpha],
                'lr': alpha_lr,
                'weight_decay': 0
            })
            print(f"✅ 已将Alpha添加到优化器，学习率: {alpha_lr}")
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        # 🔥 在这里添加梯度诊断（反向传播之后）
        if hasattr(model, 'fusion_module') and accumulated_iter % 50 == 0:
            print(f"\n=== Epoch {cur_epoch}, Iter {accumulated_iter} 梯度诊断 ===")

            # # 检查图像→点云attention梯度
            # for i, fusion_module in enumerate(model.fusion_module.fusion_modules):
            #     for name, param in fusion_module.img_cross_attn.named_parameters():
            #         if param.grad is not None:
            #             grad_norm = param.grad.norm().item()
            #             print(f"Scale{i} 图像→点云 {name}: {grad_norm:.8f}")
            #
            #     for name, param in fusion_module.lidar_cross_attn.named_parameters():
            #         if param.grad is not None:
            #             grad_norm = param.grad.norm().item()
            #             print(f"Scale{i} 点云→图像 {name}: {grad_norm:.8f}")
        clip_grad_norm_(model.parameters(), optim_cfg.GRAD_NORM_CLIP)
        # print(f"Alpha梯度: {model.alpha.grad.item():.8f}")
        # # 检查alpha是否在优化器中
        # for i, group in enumerate(optimizer.param_groups):
        #     alpha_in_group = any(p is model.alpha for p in group['params'])
        #     print(f"参数组{i}: Alpha在其中? {alpha_in_group}")

        # # 🔥 在这里添加alpha梯度放大代码
        # if hasattr(model, 'alpha') and model.alpha.grad is not None:
        #     alpha_lr_multiplier = optim_cfg.get('ALPHA_LR_MULTIPLIER', 100.0)
        #     # print('alpha_lr_multiplier',alpha_lr_multiplier)
        #     model.alpha.grad.data *= alpha_lr_multiplier
        #     if accumulated_iter % 2 == 0:  # 每50步打印一次
        #         print(f"🔧 放大Alpha梯度 {alpha_lr_multiplier}倍, 当前梯度: {model.alpha.grad.item():.6f}")
