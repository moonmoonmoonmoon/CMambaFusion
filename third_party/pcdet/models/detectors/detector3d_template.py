    ...
    def post_processing(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                ...
                # ── DEBUG ──
                print(f"[DEBUG] index={index}, cls_preds max={cls_preds.max().item():.4f}, "
                      f"min={cls_preds.min().item():.4f}")
                print(f"[DEBUG] boxes before NMS={cls_preds.shape[0]}, after NMS={selected.shape[0]}")
