...
class PointPillar(Detector3DTemplate):
    ...
    def _init_multimodal_fusion(self):
        ...
        try:
            ...
            if self.model_cfg.get('ENABLE_MULTIMODAL_FUSION', False):
                ...
                fusion_path = os.path.join(project_root, 'fusion')

                print(f"🎯 项目根目录: {project_root}")
                print(f"🎯 Fusion路径: {fusion_path}")

                # 验证路径是否存在
                if not os.path.exists(fusion_path):
                    raise FileNotFoundError(f"Fusion路径不存在: {fusion_path}")

                # 清理其他项目的fusion路径
                sys.path = [p for p in sys.path if not (p.endswith('fusion') or '/fusion' in p)]

                # 清除已导入的fusion模块
                modules_to_remove = [key for key in sys.modules.keys() if key.startswith('fusion.')]
                for module in modules_to_remove:
                    del sys.modules[module]

                # 将当前项目的fusion路径插入到最前面
                ...
                from fusion.wrappers.yolo_extractor import YOLOv8FeatureExtractor, ImagePreprocessor
                from fusion.attention.fusion_module import MultiModalFusionForPointPillars

                # 测试：打印类的模块信息
                print(f"🔍 YOLOv8FeatureExtractor 来自: {YOLOv8FeatureExtractor.__module__}")
                ...
                # from fusion.heads.yolo_detection_head import SimplifiedYOLOHead

                # 获取图像特征的输出通道数
                yolo_feature_channels = [128, 256, 512]  # P3, P4, P5

                # self.yolo_detection_head = SimplifiedUltralyticsDetector(
                #     feature_channels=yolo_feature_channels,
                #     num_classes=4,  # 使用相同的类别数
                #     # img_size=img_size
                # )
    ...
    # def _get_pointpillar_feature_info(self):
    #     """获取PointPillars特征信息"""
    #     # 根据backbone配置计算特征信息
    #     backbone_cfg = self.model_cfg.BACKBONE_2D
    #     layer_channels = list(backbone_cfg.NUM_UPSAMPLE_FILTERS)
    #
    #     return {
    #         'channels': layer_channels,
    #         'num_scales': len(layer_channels)
    #     }
    ...
    def _apply_multimodal_fusion(self, batch_dict):
        ...
        try:
            ...
            # # 记录原始特征
            # original_mean = batch_dict['spatial_features_2d'].mean().item()
            # print(f"融合前特征均值: {original_mean:.6f}")
