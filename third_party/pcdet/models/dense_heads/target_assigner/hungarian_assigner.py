...
def height_overlaps(boxes1, boxes2):
    boxes1_top_height    = (boxes1[:,2] + boxes1[:,5]/2).view(-1, 1)
    boxes1_bottom_height = (boxes1[:,2] - boxes1[:,5]/2).view(-1, 1)
    boxes2_top_height    = (boxes2[:,2] + boxes2[:,5]/2).view(1, -1)
    boxes2_bottom_height = (boxes2[:,2] - boxes2[:,5]/2).view(1, -1)
...
class HungarianAssigner3D:
    ...
    def assign(self, bboxes, gt_bboxes, gt_labels, cls_pred, point_cloud_range):
        ...
        if num_gts > 0 and not hasattr(self, '_iou_debug'):
            self._iou_debug = True

            b = bboxes[:5]
            g = gt_bboxes[:5]

            # 1. height overlap
            h_overlap = height_overlaps(b, g)
            print(f"=== height_overlaps ===\n{h_overlap}")

            # 2. BEV overlap
            overlaps_bev = b.new_zeros((b.shape[0], g.shape[0])).cuda()
            iou3d_nms_cuda.boxes_overlap_bev_gpu(
                b[:, :7].contiguous().cuda(),
                g[:, :7].contiguous().cuda(),
                overlaps_bev
            )
            print(f"=== BEV overlaps ===\n{overlaps_bev}")

            # 3. 完整IoU
            iou_cost, iou = self.iou3d_cost(b, g)
            print(f"=== 3D IoU ===\n{iou}")

            # 4. box数值
            print(f"pred boxes前5:\n{b}")
            print(f"gt boxes前5:\n{g}")
        ...
        if num_gts == 0 or num_bboxes == 0:
            ...
            if num_gts == 0:
                ...
                assigned_gt_inds[:] = 0
                max_overlaps = bboxes.new_zeros((num_bboxes,))  # ← 加这一行
        ...
        return assigned_gt_inds, max_overlaps
        # return num_gts, assigned_gt_inds, max_overlaps, assigned_labels  # ← 补全
