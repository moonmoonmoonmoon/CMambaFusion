...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        # print('4', self.root_path)
        # print('7',self.root_path.resolve())
    ...
    def prepare_data(self, data_dict):
        ...
        if self.training:
            ...
            # print('self.data_augmentor: ',self.data_augmentor)
        ...
        # print("before processor gt:", data_dict["gt_boxes"].shape)
        # data_dict = self.data_processor.forward(data_dict=data_dict)
        # print("after  processor gt:", data_dict["gt_boxes"].shape)
    ...
    def only_prepare_data(self, data_dict):
        """
        Args:
            data_dict:
                points: optional, (N, 3 + C_in)
        ...
        #     return self.__getitem__(new_index)
        if self.training and len(data_dict['gt_boxes']) == 0:
            self.empty_gt_boxes_count += 1  # 🔥 统计
            print(f"⚠️ 空样本 #{self.empty_gt_boxes_count}: {data_dict['frame_id']}")
            data_dict['gt_boxes'] = np.zeros((0, 8), dtype=np.float32)
