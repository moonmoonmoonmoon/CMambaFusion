...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None, gt_name_remap=None, data_type=None):
        self.root_path = root_path
        # print('6',self.root_path)
        self.class_names = class_names
        self.sampler_cfg = sampler_cfg
        self.gt_name_remap = gt_name_remap
        ...
        self.aug_by_scene = sampler_cfg.get('AUG_BY_SCENE', False)

        for db_info_path in sampler_cfg.DB_INFO_PATH:
            # print('1',sampler_cfg.DB_INFO_PATH, '2', db_info_path)
            # print('5',self.root_path)
            # print('3',self.root_path.resolve())
            ...
            with open(str(db_info_path), 'rb') as f:
                ...
                if self.gt_name_remap is not None:
                    for class_name_ori in infos.keys():
                        cur_class = self.gt_name_remap[class_name_ori]
                        if cur_class in class_names:
                            self.db_infos[cur_class].extend(infos[class_name_ori])
                else:
                    [self.db_infos[cur_class].extend(infos[cur_class]) for cur_class in class_names if
                     cur_class in list(infos.keys())]
        ...
        if self.aug_by_scene:
            assert isinstance(sampler_cfg.SAMPLE_GROUPS,
                              dict), 'DataBaseSampler Error: sampler_cfg.SAMPLE_GROUPS should be dict when aug_by_scene is True.'
            for elem_scene, sample_groups_cfg in sampler_cfg.SAMPLE_GROUPS.items():
                self.sample_class_num[elem_scene], self.sample_groups[elem_scene] = {}, {}
                self.sample_class_num[elem_scene], self.sample_groups[elem_scene] = self.init_sample_groups(
                    sample_groups_cfg, self.db_infos[elem_scene], self.sample_class_num[elem_scene],
                    self.sample_groups[elem_scene], class_names)
        else:
            self.sample_class_num, self.sample_groups = self.init_sample_groups(sampler_cfg.SAMPLE_GROUPS,
                                                                                self.db_infos, self.sample_class_num,
                                                                                self.sample_groups, class_names)

    def init_sample_groups(self, sample_groups_cfg, class_db_infos, sample_class_num, sample_groups, class_names):
        for x in sample_groups_cfg:
            ...
            sample_groups[class_name] = {
                ...
                'pointer': len(class_db_infos[class_name]),
                'indices': np.arange(len(class_db_infos[class_name]))
            }
        return sample_class_num, sample_groups
    ...
    def filter_by_size(self, db_infos, size_dict):
        for name, v in size_dict.items():
            if v['remap'] != 'unknown':
                remap_infos = []
                remap_flag = True
            else:
            ...
            filtered_infos = []
            for info in db_infos[name]:
                box3d_lidar = info['box3d_lidar']
                l, w, h = box3d_lidar[3], box3d_lidar[4], box3d_lidar[5]
                if l > v['l'] or w > v['w'] or h > v['h']:
                    if remap_flag:
            ...
            if self.logger is not None:
                self.logger.info('Database filter by box sizes %s: %d => %d' %
                                 (name, len(db_infos[name]), len(filtered_infos)))
                if remap_flag:
                    self.logger.info('Database added by box sizes %s: %d => %d' %
                                     (v['remap'], len(db_infos[v['remap']]),
                                      len(db_infos[v['remap']]) + len(remap_infos)))
            db_infos[name] = filtered_infos
            if remap_flag:
                self.db_infos[v['remap']].extend(remap_infos)
        return db_infos

    def split_by_scene_category(self, db_infos, split_scene_list):
        new_db_infos = {}
        for elem_scene in split_scene_list:
            new_db_infos[elem_scene] = {}

            for key, dinfos in db_infos.items():
                pre_len = len(dinfos)
                new_db_infos[elem_scene][key] = [
                    info for info in dinfos
                    if info['scene_elements']['scene_category'] == elem_scene
                ]
                if self.logger is not None:
                    self.logger.info('Database split by scene category %s - %s: %d => %d' %
                                     (elem_scene, key, pre_len, len(new_db_infos[elem_scene][key])))
        return new_db_infos
    ...
    @staticmethod
    def put_boxes_on_road_planes_twdto(gt_boxes, road_planes, road_points_num, road_points_num_thred=1000):
        """
        Validate in twdto dataset
        Args:
            gt_boxes: (N, 7 + C) [x, y, z, dx, dy, dz, heading, ...]
            road_planes: [a, b, c, d]
            calib:
        ...
        Returns:
        """
        if road_points_num < road_points_num_thred:
            return gt_boxes, np.zeros(gt_boxes.shape[0])

        a, b, c, d = road_planes

        cur_lidar_height = (-d - a * gt_boxes[:, 0] - b * gt_boxes[:, 1]) / c
        mv_height = gt_boxes[:, 2] - gt_boxes[:, 5] / 2 - cur_lidar_height
        gt_boxes[:, 2] -= mv_height  # lidar view
        return gt_boxes, mv_height
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None,
        ...
        if self.sampler_cfg.get('USE_ROAD_PLANE', False) and mv_height is None:
            if self.data_type == "twdto":
                sampled_gt_boxes, mv_height = self.put_boxes_on_road_planes_twdto(
                    sampled_gt_boxes, data_dict['plane']['road_plane'], data_dict['plane']['road_points_num'],
                    self.sampler_cfg.get('ROAD_POINTS_THRED', 1000)
                )
                ...
                raise TypeError("USE_ROAD_PLANE in data augmentation not support data_type : {}".format(self.data_type))
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            if self.use_shared_memory:
                ...
                # print('1', np.fromfile(str(file_path), dtype=np.float32).shape)
        ...
        if self.gt_name_remap is not None:
            sampled_gt_names = np.array([self.gt_name_remap[x['name']] for x in total_valid_sampled_dict])
        else:
    ...
    def __call__(self, data_dict):
        ...
        if self.aug_by_scene:
            assert "common_info" in data_dict and "scene_elements" in data_dict[
                "common_info"], 'DataBaseSampler Error: Not find scene_elements in data_dict.'
            frame_elem_scene = data_dict["common_info"]["scene_elements"]["scene_category"]
            frame_sample_class_num, frame_sample_groups, frame_db_infos = self.sample_class_num[frame_elem_scene], \
            self.sample_groups[frame_elem_scene], self.db_infos[frame_elem_scene]
        else:
            frame_sample_class_num, frame_sample_groups, frame_db_infos = self.sample_class_num, self.sample_groups, self.db_infos

        for class_name, sample_group in frame_sample_groups.items():
            # # 检查该类别是否有可用样本
            # if len(frame_db_infos.get(class_name, [])) == 0:
            #     if self.logger is not None:
            #         self.logger.debug(f'No database samples available for class {class_name}, skipping')
            #     continue
            if self.limit_whole_scene:
                num_gt = np.sum(class_name == gt_names)
                sample_group['sample_num'] = str(int(frame_sample_class_num[class_name]) - num_gt)
            if int(sample_group['sample_num']) > 0:
                sampled_dict = self.sample_with_fixed_number(class_name, sample_group, frame_db_infos)
                # 检查采样结果是否为空
                if len(sampled_dict) == 0:
                    if self.logger is not None:
                        self.logger.debug(f'No samples obtained for class {class_name}')
                    continue
