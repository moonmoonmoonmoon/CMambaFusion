...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        ...
        # # 统计每帧的GT和DT数量
        # total_gt_count += gt_datas.shape[0]
        # total_dt_count += dt_datas.shape[0]

    total_dc_num = np.stack(total_dc_num, axis=0)
    # # 添加打印统计信息
    # print(f"=== _prepare_data Debug Info ===")
    # print(f"Current class: {current_class}, Difficulty: {difficulty}")
    # print(f"Total valid GT: {total_num_valid_gt}")
    # print(f"Total GT boxes: {total_gt_count}")
    # print(f"Total DT boxes: {total_dt_count}")
    # print(f"Total frames: {len(gt_annos)}")
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                # # 最后打印最终结果
                # print(f"=== Class {current_class}, Difficulty {difficulty}, Min_overlap {min_overlap:.2f} ===")
                # print(f"Thresholds shape: {thresholds.shape}")
                # print(f"PR array shape: {pr.shape}")
                # print("Threshold | TP    | FP    | FN    | Precision | Recall")
                # print("-" * 60)
                # for i in range(len(thresholds)):
                #     tp = pr[i, 0]  # TP/FP/FN不会被单调化影响
                #     fp = pr[i, 1]
                #     fn = pr[i, 2]
                #     prec = precision[m, l, k, i]  # 这是单调化后的precision
                #     rec = recall[m, l, k, i]  # 这是单调化后的recall
                #     print(f"{thresholds[i]:.3f}   | {tp:5.0f} | {fp:5.0f} | {fn:5.0f} | {prec:.3f}     | {rec:.3f}")
    ret_dict = {
        "recall": recall,
        "precision": precision,
        "orientation": aos,
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    overlap_0_7 = np.array([[0.7, 0.7, 0.5, 0.7, 0.7, 0.7, 0.7, 0.5, 0.7,
                             0.7, 0.5], [0.7, 0.7, 0.5, 0.7, 0.7, 0.7, 0.7, 0.5, 0.7,
                             0.7, 0.5], [0.7, 0.7, 0.5, 0.7, 0.7, 0.7, 0.7, 0.5, 0.7,
                             0.7, 0.5]])
    overlap_0_5 = np.array([[0.7, 0.7, 0.5, 0.7, 0.7, 0.7, 0.7, 0.5, 0.7,
                             0.7, 0.5], [0.5, 0.5, 0.25, 0.5, 0.5, 0.5, 0.5, 0.25, 0.5, 0.5, 0.25],
                            [0.5, 0.5, 0.25, 0.5, 0.5, 0.5, 0.5, 0.25, 0.5, 0.5, 0.25]])
