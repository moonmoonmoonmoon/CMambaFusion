...
class YOLODataset(BaseDataset):
    ...
    def build_transforms(self, hyp=None):
        ...
        import traceback
        print("=" * 50)
        print(f"build_transforms 调试:")
        print(f"self.augment: {self.augment}")
        print(f"调用位置: {traceback.extract_stack()[-2].filename}:{traceback.extract_stack()[-2].lineno}")
        print("=" * 50)

        if self.augment:
            print('self.imgz when augment:',self.imgsz)
            hyp.mosaic = hyp.mosaic if self.augment and not self.rect else 0.0
            hyp.mixup = hyp.mixup if self.augment and not self.rect else 0.0
            hyp.cutmix = hyp.cutmix if self.augment and not self.rect else 0.0
            transforms = v8_transforms(self, self.imgsz, hyp)
        else:
            print('no augment,self.imgz',self.imgsz)
