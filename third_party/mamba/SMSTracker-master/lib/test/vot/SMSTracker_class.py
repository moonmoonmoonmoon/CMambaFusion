...
class SMSTracker(object):
    def __init__(self, tracker_name='', para_name=''):
        # create tracker
        cfg = env_setting(para_name)
        tracker_info = Tracker(tracker_name, para_name, cfg.test.dataset_name, yaml_name=para_name)
...
def run_vot_exp(tracker_name, para_name, vis=False, out_conf=False, channel_type='color'):
    ...
    if vis and (not os.path.exists(save_root)):
        os.mkdir(save_root)
    tracker = SMSTracker(tracker_name=tracker_name, para_name=para_name)
