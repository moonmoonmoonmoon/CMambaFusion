...
def load_dataset(name: str,yaml_name:str):
    ...
    dataset = getattr(m, dset_info.class_name)(**dset_info.kwargs,yaml_name = yaml_name)  # Call the constructor
    return dataset.get_sequence_list()


def get_dataset(yaml_name:str,*args):
    """ Get a single or set of datasets."""
    dset = SequenceList()
    for name in args:
        dset.extend(load_dataset(name,yaml_name))
    return dset