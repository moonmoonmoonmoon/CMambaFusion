...
import numpy as np
from lib.dataset.depth_utils import get_rgbd_frame


def trackerlist(name: str, parameter_name: str, dataset_name: str, yaml_name: str, run_ids=None, display_name: str = None,
                result_only=False, ):
    ...
    if run_ids is None or isinstance(run_ids, int):
        run_ids = [run_ids]
    return [Tracker(name, parameter_name, dataset_name, yaml_name, run_id, display_name, result_only) for run_id in run_ids]
...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, dataset_name: str, yaml_name: str, run_id: int = None, display_name: str = None,
                 result_only=False):
        ...
        if yaml_name is None:
            raise ValueError("yaml_name is None")
        cfg = env_setting(yaml_name)
        if self.run_id is None:
            self.results_dir = '{}/{}/{}'.format(cfg.analysis.results_path,self.dataset_name, self.parameter_name)
        else:
