...
class LTRLoader(torch.utils.data.dataloader.DataLoader):
    ...
    def __init__(self, name, dataset, training=True, batch_size=1, shuffle=False, sampler=None, batch_sampler=None,
        ...
        :param shuffle : (bool, optional) set to ``True`` to have the data reshuffled at every epoch (default: False).
        :param sampler : (Sampler, optional) defines the strategy to draw samples from the dataset. If specified, ``shuffle`` must be False.
        :param batch_sampler (Sampler, optional):like sampler, but returns a batch of indices at a time. Mutually exclusive with batch_size, shuffle, sampler, and drop_last.
        :param num_workers :(int, optional)how many subprocesses to use for data loading. 0 means that the data will be loaded in the main process. (default: 0)
        :param epoch_interval:
        :param collate_fn :(callable, optional)merges a list of samples to form a mini-batch.
        :param stack_dim:(int) Dimension along which to stack to form the batch. (default: 0)
        :param pin_memory: (bool, optional) If ``True``, the data loader will copy tensors into CUDA pinned memory before returning them.
        ...
        :param timeout: (numeric, optional)if positive, the timeout value for collecting a batch from workers. Should always be non-negative. (default: 0)
        :param worker_init_fn: (callable, optional)  If not None, this will be called on each worker subprocess with the worker id (an int in ``[0, num_workers - 1]``) as input, after seeding and before data loading. (default: None)
        """
        ...
        super(LTRLoader, self).__init__(dataset, batch_size, shuffle, sampler, batch_sampler, num_workers, collate_fn, pin_memory, drop_last, timeout,
                                        worker_init_fn)
